...
compute_lambda_gcv <- function(lambda_mu_flag, design_matrix_Z, W, w, z, K, lambda_acc, n) {
  ...
  for(j in 1:lambda_acc) {
    ...
    if(lambda_mu_flag) {
      parameter_hat <- solve(t(design_matrix_Z) %*% W %*% design_matrix_Z + lambda_gcv[j] * K) %*% t(design_matrix_Z) %*% W %*% z
    ...
    S <- design_matrix_Z %*% solve(t(design_matrix_Z) %*% W %*% design_matrix_Z + lambda_gcv[j] * K) %*% t(design_matrix_Z) %*% W
